...
export default function SignupPage() {
    ...
    const handleSignup = async (e: React.FormEvent) => {
        ...
        try {
            ...
            // If user.identities is empty, it means the email is already registered
            if (data.user && data.user.identities && data.user.identities.length === 0) {
                setError('An account with this email already exists. Please sign in instead.')
                return
            }

            // Always show confirmation message for email signup
            ...
            <div className="w-full max-w-md bg-white rounded-xl p-8 shadow-2xl">
                ...
                {success && (
                    <div className="text-center space-y-4">
                        ...
                        <p className="text-slate-400 text-xs">
                            ...
                            <button onClick={() => setSuccess(false)} className="text-cyan-600 hover:underline">
                                try again
                            </button>
                        </p>
                    </div>
                ...
                {!success && (<>
                    ...
                    <form onSubmit={handleSignup} className="space-y-4">
                        ...
                        <div className="space-y-2">
                            ...
                            <div className="relative">
                                <input
                                    ...
                                    className="w-full bg-slate-50 border border-slate-300 rounded-lg px-3 py-2 text-slate-900 focus:outline-none focus:border-cyan-500 focus:ring-1 focus:ring-cyan-500 pr-10 transition-colors placeholder:text-slate-400"
                                    placeholder="••••••••"
                                />
                                <button
                                    type="button"
                        ...
                        {error && (
                            <div className={`p-3 border rounded-lg text-sm ${error.includes('check your email') ? 'bg-green-50 border-green-200 text-green-700' : 'bg-red-50 border-red-200 text-red-600'}`}>
                                {error}
                            </div>
                        )}
